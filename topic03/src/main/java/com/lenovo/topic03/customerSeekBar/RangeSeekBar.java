package com.lenovo.topic03.customerSeekBar;
...
public class RangeSeekBar extends View {
    ...
    //一个单元格的单位
    private int oneItemValue = 5;
    //一个刻度的单位
    private int oneGroupValue = 30;
    //一个单元格的宽度
    ...
    public RangeSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        widthOfItem = 10/*(getScreenWidth(getContext().getApplicationContext()) - PADDING_LEFT - PADDING_RIGHT) / numberOfItem*/;
        //总宽减去线的宽度
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.i("--++", scaleX + "//////////" + scaleY);
        mTextPaint.setTextSize(12 * scaleX);
        mLinePaint.setStrokeWidth(widthOfLine);
        mRulerPaint.setStrokeWidth(widthOfLine);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = minProgress; i <= maxProgress; i = i + oneItemValue) {
            if (i % oneGroupValue == 0 || i == minProgress) {
                ...
                //画毫米线
                double lineHeight = (getHeight() / 2) * 8;
...
//        canvas.drawText(progrees+"", getWidth()/2-cursorTextWidth/2, cursorTextWidth, mTextPaint);
    ...
    public static double getScreenInch(Activity context) {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= 17) {
                ...
                Method mGetRawH = Display.class.getMethod(new String("getRawHeight"));
                Method mGetRawW = Display.class.getMethod(new String("getRawWidth"));
    ...
    public void setCurrentProgress(int currentProgress) {
        ...
        xProgress = (float) (currentProgress / oneItemValue - minProgress / oneItemValue) * widthOfItem + PADDING_LEFT;
        invalidate();
        if (onRangeRulerChangeListener != null) {
